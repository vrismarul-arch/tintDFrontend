...
export default function CategoryServices() {
  ...
  const initialSubCat = queryParams.get("subcat");
    ...
    const fetchData = async () => {
      try {
        const res = await api.get(
          `/api/admin/services?category=${id}&status=active`
        );

        setServices(res.data || []);

        // ⭐ SAFE extraction of subcategories & varieties
        const subs = [];
        const vars = [];

        (res.data || []).forEach((s) => {
          if (s?.subCategory?._id && !subs.some((x) => x._id === s.subCategory._id)) {
            subs.push(s.subCategory);
          }
          if (s?.variety?._id && !vars.some((x) => x._id === s.variety._id)) {
            vars.push(s.variety);
          }
        });
  ...
  // ⭐ FILTER ACTIVE SERVICES BY SUBCATEGORY & VARIETY
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "discount",
      render: (discount) =>
        discount ? <Tag color="green">{discount}% OFF</Tag> : <Tag>—</Tag>,
    },
    {
      title: "Action",
      render: (_, record) => (
        <div className="action-buttons">
          {isInCart(record._id) ? (
            <Button
              ...
              shape="round"
              size="small"
              onClick={() => handleRemoveFromCartClick(record._id)}
            >
              REMOVE
            </Button>
          ) : (
